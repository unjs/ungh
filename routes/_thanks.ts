...
async function _renderSVG(users: { login: string; avatar: string }[]) {
  ...
  <a href="https://vercel.com" target="_blank"><text x="105" y="50" text-anchor="middle" class="link" font-family="-apple-system, system-ui, sans-serif" font-size="11">Hosted on</text><g transform="translate(140,34) scale(0.32)"><path class="link" d="${VERCEL_LOGOTYPE_PATH}"/></g></a>
  <text x="150" y="74" text-anchor="middle" class="heading" font-family="-apple-system, system-ui, sans-serif" font-size="13" font-weight="600" letter-spacing="0.05em">TOKEN DONORS</text>
  ...
  const items = users
    .map((user, i) => {
      ...
      const avatarY = y;
      const textY = y + AVATAR_SIZE + 14;
      const truncated = user.login.length > 10 ? user.login.slice(0, 9) + "…" : user.login;
    ...
    <text x="${x}" y="${textY}" text-anchor="middle" class="label" font-family="-apple-system, system-ui, sans-serif" font-size="11">${_escapeXml(truncated)}</text>
  </a>`;
    })
    .join("\n");
  ...
  <a href="https://vercel.com" target="_blank"><text x="${width / 2 - 45}" y="50" text-anchor="middle" class="link" font-family="-apple-system, system-ui, sans-serif" font-size="11">Hosted on</text><g transform="translate(${width / 2 - 10},34) scale(0.32)"><path class="link" d="${VERCEL_LOGOTYPE_PATH}"/></g></a>
  <text x="${width / 2}" y="74" text-anchor="middle" class="heading" font-family="-apple-system, system-ui, sans-serif" font-size="13" font-weight="600" letter-spacing="0.05em">TOKEN DONORS</text>
${items}
</svg>`;
}
